...
根据我解析语料的经验，在获得XML文档数的根节点后，实际上分为两种节点(这里测试只用到这两种节点，实际按照nodeType的节点还有很多):元素节点(ELEMENT NODE)和文本节点(TEXT NODE)。元素节点如上面的 "DOCID" 标签，整个就是一个元素节点。文本节点如上面的“ CNN_CF_20030303.1900.00”,也作为一个节点，即文本节点。
...
文本节点(TEXT NODE)可以用 colume.getAttribute('Name')这样来获取Name的这样一个属性值，属性值指的是:<Column Name='pt' Value='1'/>这样的结构。可以使用node.data或者node.nodeValue来获取文本值。
...
- doc_root获得的是整个xml对象。
- child是标签<TEXT>下所有element节点和text节点。
- 获取标签<TURN>下面的文本内容，c这里是类似一个列表的东西，可以用列表方法获取，它是有孩子节点的。
...
>		em_id=em.attrib['ID]                         start=em.find('head').find('charseq').attrib['START']                    text=em.find('head').find('charseq').text.replace('\n'," ")          
>```
