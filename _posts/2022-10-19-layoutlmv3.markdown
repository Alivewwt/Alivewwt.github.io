...
- 任务3  文本视觉问答 文本视觉问答接收文本图像和问题作为输入，输出答案，将该任务定义成一个QA问题。在DocVQA数据集上进行实验，使用ANLS(也称为平均归一化)Levenshtein similarity，$LayoutLMv3\_{BASE}比LayoutLMv2\_{BASE}$的ANLS分数从78.08提高至78.76，$LayoutLMv3_{LARGE}比LayoutLMv2_{BASE}$取得了4.6的绝对收益。
