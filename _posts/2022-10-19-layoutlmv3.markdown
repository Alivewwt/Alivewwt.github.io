...
- 任务3 文本视觉问答 文本视觉问答接收文本图像和问题作为输入，输出答案，将该任务定义成一个QA问题。在DocVQA数据集上进行实验，使用ANLS（也称为平均归一化Levenshtein similarity）,$LayoutLMv3_{BASE}$比$LayoutLMv2_{BASE}$的ANLS分数从$78.08$提高至$78.76$，$LayoutLMv3_{LARGE}$比$LayoutLMv2_{BASE}$取得了$4.61$的绝对收益。
